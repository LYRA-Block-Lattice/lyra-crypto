...
import { APIResult, BlockAPIResult, MultiBlockAPIResult, NewTransferAPIResult2, SimpleJsonAPIResult } from "./blocks/meta";
...
BlockchainAPI.getUnreceived = (accountId) => _a.fetchJson2(NewTransferAPIResult2, `${_a.Block_API_v1}/LookForNewTransfer2?AccountId=${accountId}`);
BlockchainAPI.findTokens = (keyword, cat) => _a.fetchJson(`${_a.Block_API_v1}/FindTokens?q=${keyword}&cat=${cat}`);
BlockchainAPI.getBlockBySourceHash = (hash) => _a.fetchJson2(BlockAPIResult, `${_a.Block_API_v1}/GetBlockBySourceHash?Hash=${hash}`);
BlockchainAPI.findFiatWallet = (owner, symbol) => _a.fetchJson2(BlockAPIResult, `${_a.Block_API_v1}/FindFiatWallet?owner=${owner}&symbol=${symbol}`);
...
BlockchainAPI.searchDao = (q) => _a.fetchJson2(MultiBlockAPIResult, `${_a.Block_API_v1}/FindDaos?q=${q}`);
...
// Dealer API
BlockchainAPI.startWallet = (accountId, signature) => _a.fetchJson2(APIResult, `${_a.Dealer_API}/WalletCreated?accountId=${accountId}&signature=${signature}`);
BlockchainAPI.getPrices = async () => _a.fetchJson2(SimpleJsonAPIResult, `${_a.Dealer_API}/GetPrices`);
